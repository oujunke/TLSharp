...
namespace Starksoft.Aspen.Proxy
{
    ...
    public class HttpProxyClient
    {
        ...
        public TcpClient CreateConnection(string destinationHost, int destinationPort)
        {
            ...
            {
                throw new Exception(String.Format(CultureInfo.InvariantCulture, "Connection to proxy host  on port  failed."), ex);
            }
        }
        ...
        private void HandleProxyCommandError(string host, int port)
        {
            ...
            switch (_respCode)
            {
                case HttpResponseCodes.None:
                    msg = String.Format(CultureInfo.InvariantCulture, "Proxy destination on port  failed to return a recognized HTTP response code.  Server response: {0}", _respText);
                    break;

                case HttpResponseCodes.BadGateway:
                    //HTTP/1.1 502 Proxy Error (The specified Secure Sockets Layer (SSL) port is not allowed. ISA Server is not configured to allow SSL requests from this port. Most Web browsers use port 443 for SSL requests.)
                    msg = String.Format(CultureInfo.InvariantCulture, "Proxy destination  on port  responded with a 502 code - Bad Gateway.  If you are connecting to a Microsoft ISA destination please refer to knowledge based article Q283284 for more information.  Server response: {0}",  _respText);
                    break;

                default:
                    msg = String.Format(CultureInfo.InvariantCulture, "Proxy destination  on port  responded with a {0} code - {1}", ((int)_respCode).ToString(CultureInfo.InvariantCulture), _respText);
                    break;
            }
        ...
        private void WaitForData(NetworkStream stream)
        {
            ...
            while (!stream.DataAvailable)
            {
                ...
                if (sleepTime > WAIT_FOR_DATA_TIMEOUT)
                    throw new Exception(String.Format("A timeout while waiting for the proxy server at  on port  to respond."));
            }
        }
        ...
        private void ParseCodeAndText(string line)
        {
            ...
            if (line.IndexOf("HTTP") == -1)
                throw new Exception(String.Format("No HTTP response received from proxy destination.  Server response: {0}.", line));
            ...
            if (!Int32.TryParse(val, out code))
                throw new Exception(String.Format("An invalid response code was received from proxy destination.  Server response: {0}.", line));
        ...
        private String GetHttpVersionString()
        {
            switch (_httpVersion)
            {
                ...
                case HttpVersions.Version1_1:
                    return "1.1";
                default:
                    throw new Exception(String.Format("Unexpect HTTP version {0} specified", _httpVersion.ToString()));
            }
        }
