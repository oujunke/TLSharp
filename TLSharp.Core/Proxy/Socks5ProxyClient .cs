...
namespace Starksoft.Aspen.Proxy
{
    ...
    public class Socks5ProxyClient
    {
        ...
        private void NegotiateServerAuthMethod()
        {
            ...
            {
                _tcpClient.Close();
                throw new Exception("The proxy destination requires a username and password for authentication.  If you received this error attempting to connect to the Tor network provide an string empty value for ProxyUserName and ProxyPassword.");
            }
            ...
            if (acceptedAuthMethod == SOCKS5_AUTH_METHOD_USERNAME_PASSWORD)
            {
                ...
                {
                    _tcpClient.Close();
                    throw new Exception("Proxy authentification failure!  The proxy server has reported that the userid and/or password is not valid.");
                }
        ...
        private byte GetDestAddressType(string host)
        {
            ...
            switch (ipAddr.AddressFamily)
            {
                ...
                default:
                    throw new Exception(String.Format(CultureInfo.InvariantCulture, "The host addess {0} of type '{1}' is not a supported address type.  The supported types are InterNetwork and InterNetworkV6.", host, Enum.GetName(typeof(AddressFamily), ipAddr.AddressFamily)));
            }
        ...
        private void HandleProxyCommandError(byte[] response, string destinationHost, int destinationPort)
        {
            ...
            string responseText = response != null ? Convert.ToBase64String(response) : "";
